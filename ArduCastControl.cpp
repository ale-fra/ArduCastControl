...
const char CC_MSG_CAST[] = "{\"type\":\"MEDIA\",\"media\":{\"url\":\"http://192.168.68.56/shared/a_ram_sam_sam.mp3\"}, \"requestId\": 2, \"mediaSessionId\": ";
...
connection_t ArduCastControl::loop()
{
  ...
  do
  {
    ...
    if (read > 0)
    {
      ...
      do
      {
        ...
        {
          if (0 == memcmp(connBuffer + offset, CC_NS_HEARTBEAT, lengthOrValue))
          { // pong message, no need to process the payload
            processPayload = 0;
          }
          if (0 == memcmp(connBuffer + offset, CC_NS_CONNECTION, lengthOrValue))
          { // must be a close message
            if (processPayload == 1)
            {
        ...
        if (processPayload > 0 && tag == extensions_api_cast_channel_CastMessage_payload_utf8_tag)
        {
          ...
          if (!error && doc.containsKey("type") && doc.containsKey("status"))
          { // it pretty much must contain it
  ...
  if (!rxProcessed && (!msgSent || ((millis() - msgSentAt) > 500)))
  {
    ...
    else if (applicationConnection.getConnectionStatus() == CH_NEEDS_PING)
    { // this will never happen, unless loop is called rarely
...
int ArduCastControl::pause(bool toggle)
{
  ...
  {
    // snprintf(cc_msg_ctrl, sizeof(cc_msg_ctrl), "%s%d}", CC_MSG_CAST, mediaSessionId);
    return applicationConnection.writeMsg(CC_NS_CAST, cc_msg_ctrl);
  }
}

int ArduCastControl::cast(String mp3)
{
  if (msgSent)
    return -10;
  if (mediaSessionId < 0)
